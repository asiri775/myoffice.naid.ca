...
@endsection
@section('footer')
    <script src="{{ asset('module/booking/js/checkout.js') }}?v=1"></script>
    <script type="text/javascript">
        ...
        $('.booking-form #accordionExample .payment_type').val('full');
        $('.deposit_amount').on('change', function(){
            $('.booking-form #accordionExample .pay_amount').val($('.deposit_amount').val());
            $('.booking-form #accordionExample .payment_type').val('partial');
            jQuery(function () {
                    var pay_amount = $('.deposit_amount').val();
                    var pre_hash=$('#pre_hash').val();
                    $.ajax({
                        'url': bookingCore.url + '/updateTxnToken',
                        'data': {pay_amount:pay_amount,pre_hash:pre_hash},
                        'cache': false,
                        'method':"post",
                        success: function (res) {
                            $('.booking-form #accordionExample .txnToken').val(res);
                        }
                    });
        ...
        function checkPaynow() {
            ...
            if (how_to_pay == 'full') {
                var pay_now_need_pay = parseFloat({{floatval($booking->total*1.13)}}) - convert_to_money;
            } else {
                var pay_now_need_pay = parseFloat({{floatval($booking->deposit == null ? ($booking->total*1.13) : $booking->deposit)}}) - convert_to_money;
            }
