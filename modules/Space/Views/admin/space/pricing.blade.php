    ...
    <div class="panel-body">
        @if(is_default_lang())
            <div class="row">
                ...
                <div class="col-lg-3">
                    <div class="form-group">
                        ...
                        <input type="number" min="0" max="100" name="discount" class="form-control" value="{{$row->discount}}" placeholder="{{__("Discount %")}}">
                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="form-group">
            ...
            <div class="row">
                <script>
              let current_price=0;
              @if($row->hourly_price_set_default)  current_price=1; @endif
              @if($row->daily_price_set_default)  current_price=2;  @endif
              @if($row->weekly_price_set_default) current_price=3;  @endif
              @if($row->monthly_price_set_default)  current_price=4;  @endif
                       ...
                       function setDefaultPrice(price_id){             
                          ...
                          jQuery.ajax({
                             ...
                             success: function(result){
                                for(var i=0; i<5;i++){
                                    $('.user-form-settings .setDefaultprice_'+i).html('<a href="#" onclick="setDefaultPrice('+i+');" >Set Default</a>');
                                }
                                if(current_price){
                                    $('.user-form-settings .viewDefaultPrice_'+current_price).html('<a href="#" onclick="setDefaultPrice('+current_price+');" >Set Default</a>');
                                }
                ...
                <table class="table" style="border-bottom:1px solid #ccc">
                    <tr>
                        ...
                        <th width="20%">Discounted Rate</th>
                        <th width="20%">Set Default</th>
                        <th width="20%"></th>
                    </tr>
                    <tr>
                    <td>{{__("Hourly")}}</td>
                    <td><input type="number" step="any" min="0" name="hourly" class="form-control" value="{{$row->hourly}}" placeholder="{{__("Hourly")}}"></td>
                    <td>${{($row->discount)?number_format((float)($row->hourly-($row->discount/100)*$row->hourly), 2, '.', ''):''}}</td>
                    ...
                    <tr>
                        <td>{{__("Daily")}}</td>
                        <td> <input type="number" min="0" max="100" name="daily" class="form-control" value="{{$row->daily}}" placeholder="{{__("Daily")}}"></td>
                        <td>${{($row->discount)?number_format((float)($row->daily-($row->discount/100)*$row->daily), 2, '.', ''):''}}</td>
                    ...
                    <tr>
                        <td>{{__("Weekly")}}</td>
                        <td> <input type="number" step="any" min="0" name="weekly" class="form-control" value="{{$row->weekly}}" placeholder="{{__("Weekly")}}"> </td>
                        <td>${{($row->discount)?number_format((float)($row->weekly-($row->discount/100)*$row->weekly), 2, '.', ''):''}}</td>
                    ...
                    <tr>
                        <td>{{__("Monthly")}}</td>
                        <td><input type="number" step="any" min="0" name="monthly" class="form-control" value="{{$row->monthly}}" placeholder="{{__("Monthly")}}"> </td>
                        <td>${{($row->discount)?number_format((float)($row->monthly-($row->discount/100)*$row->monthly), 2, '.', ''):''}}</td>
