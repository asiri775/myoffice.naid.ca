...
class HomeController extends FrontendController
{
    ...
    public function spaceDeatils(Request $request, $space_id)
    {
        ...
        $space_ids_of_category = SpaceTerm::where('target_id', $space_id)->pluck('target_id');

        if (count($space_ids_of_category) > 0) {

            $similar_space = Space::whereIn('id', $space_ids_of_category)->where('id', '!=', $space_id)->limit(5)->get();
            if ($similar_space) {
    ...
    public function bookingList(Request $request)
    {
        ...
        if (!$user) {
            ...
            if ($searched_from_date) {
                $booking_list = $booking_list->where('start_date', '>', $searched_from_date);
            }

            if (CodeHelper::isNotEmpty($searched_to_date)) {
                $booking_list = $booking_list->where('end_date', '<', $searched_to_date);
            }
            if ($searched_space_name) {
                $space_ids = Space::where('title', 'like', '%' . $searched_space_name . '%')->pluck('id');
                $booking_list = $booking_list->whereIn('object_id', $space_ids);
            }
            if ($searched_city) {
            ...
            if ($searched_list_id) {
                $booking_list = $booking_list->where('code', $searched_list_id);
            }
            if ($searched_amount_from) {
                $booking_list = $booking_list->where('total', '>', $searched_amount_from);
            }
            if ($searched_amount_to) {
                $booking_list = $booking_list->where('total', '<', $searched_amount_to);
            }
