<form class="form bravo-form-login" method="POST" action="{{ route('login') }}">
    @csrf
    ...
    <div class="fulwidthm forget-signup">
        <div class=" loginrow   robotoregular fontsize12 graytext mgnB15">
            <a href="{{ route("password.request") }}" class="greentext frgtpaswrdclick font-weight-bold" onclick="forgotpassword()">{{__('Forgot Password?')}}</a>
        </div>
        <div class=" loginrow graytext mgnB15 icon-center">
            <i class="greentext ">|</i>
        </div>
        <div class="loginrow   robotoregular fontsize12 graytext mgnB15">
            <a href="{{ route("auth.register") }}"  class="greentext  font-weight-bold">{{__('Sign Up')}}</a>
        </div>
    </div>
