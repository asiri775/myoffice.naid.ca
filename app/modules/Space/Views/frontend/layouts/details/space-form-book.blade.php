    ...
    <div class="detailsbooking fulwidthm left mgnB10 pdgB15 dobordergry mt-5 mb-3">
        <div class="input-daterange" id="datepicker">
            <div class="detailformrow mgnB15 left fulwidthm avl-cal" style="text-align: center">
                <a href="javascript:;" id="openCalendar">Click Here to See Availablity Calendar</a>
            </div>
            <div class="detailformrow mgnB15 left fulwidthm">
                <div class="dateInputC">
                    ...
                    <input id="dpd1x1" class="start_date fulwidthm text-left whitebg detailsinput" name="start"
                           placeholder="Check In" type="text" value="" readonly>
                </div>
                        ...
                        @foreach($allDayTimeSlots as $slot)
                            @if( ($slot >= $row->available_from) && ($slot <= $row->available_to) )
                                <option
                                    ...
                                    @elseif($row->available_from == $slot) selected
                                    @endif value="{{$slot}}">{{ date("h:i", strtotime($slot)) }}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                <div class="whenInputC">
                    <select class="selectsearch"
                        ...
                        @php
                            $start_hour_state = date("A",strtotime($row->available_from));
                        @endphp
            ...
            <div class="detailformrow mgnB15 left fulwidthm">
                <div class="dateInputC">
                    ...
                    <input id="dpd2x2" class="end_date fulwidthm  text-left whitebg detailsinput" name="end"
                           placeholder="Check Out" type="text" value="" readonly>
                </div>
                <div class="timeInputC" style="@if(!$showTimingOption) display:none !important; @endif">
                        ...
                        @foreach($allDayTimeSlots as $slot)
                            @if( ($slot >= $row->available_from) && ($slot <= $row->available_to) )
                                <option
                                    ...
                                    @elseif($row->available_to == $slot) selected
                                    @endif value="{{$slot}}">  {{ date("h:i", strtotime($slot)) }}</option>
                            @endif
                        @endforeach
                    </select>
                ...
                <div class="whenInputC">
                    <select class="selectsearch"
                            ...
                            id="end_ampm">
                        @php
                            $end_hour_state = date("A",strtotime($row->available_to));
                        @endphp
...
<div class="extra-box mt-3 mb-3">
    ...
    <div class="form-section-group form-group">
        ...
        <div class="submit-group">
            <a name="submit" class="btn btn-large" onclick="addToCart();"><span>BOOK NOW</span></a>
        </div>
    </div>
</div>
<div class="clearfix"></div>
...
<script>
    ...
    function checkTimeAvailability(showAlerts = true) {
        ...
        $.post("{{ route('space.vendor.availability.verifySelectedTimes') }}", {
            id: {{ $row->id }},
            start_date: $('#dpd1x1').val(),
            end_date: $('#dpd2x2').val(),
            start_hour: $('select[name="start_hour"]').val(),
            end_hour: $('select[name="end_hour"]').val(),
            start_ampm: $("#start_ampm option:selected").val(),
            end_ampm: $("#end_ampm option:selected").val(),
        }, function (response) {
    ...
    function jqueryLoaded() {
        ...
        $(document).on("change", 'input[class="start_date"]', function () {
